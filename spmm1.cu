 ...
 */
int b_search(int start,int ender,int *arr,int val){
    if(start==ender-1)
        return start;
    else{
...
double sample(csr mat1,int *d_mat1_c,int *d_mat1_r,int *d_mat1_edges,int *partOut,int numRows,int numCols,cudaStream_t &stream1,bool *hflg,bool *dflg,float *prg,float percent,int *load){
	double t1,t2;
	...
	t1=omp_get_wtime();
	gpuSample<<<gridsize,blocksize,0,stream1>>>(d_mat1_c,d_mat1_r,d_mat1_edges,d_mat1_c, d_mat1_r, d_mat1_edges,numRows, partOut,part,dflg);
	...
	cudaDeviceSynchronize();
	hflg[0]=false;
	//extrapolate the proper division point
	//return timeEstimate(t2-t1,(cpudone*1.0)/(part*1.0),prg[0]);
	return t2-t1;
}

float sampleSearch(csr mat1,int *d_mat1_c,int *d_mat1_r,int *d_mat1_edges,int *partOut,int numRows,int numCols,int *h_load){
